...
export class BuyerService {
  ...
  public getBidsForBuyer(buyerId): Observable<Bid[]> {
    return this.http.get<Bid[]>(environment.hostName + `/buyer/${buyerId}/bids`);
  }

  public createBid(bid: Bid): Observable<any> {
    return this.http.post<any>(environment.hostName + `/buyer/${bid.buyerId}/bids`, bid);
  }

  public updateBid(bid: Bid): Observable<any> {
    return this.http.put<any>(environment.hostName + `/bids/${bid.id}`, bid);
  }
